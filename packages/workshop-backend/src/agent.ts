...
export interface AgentHooks {
  ...
  /**
   * Platform-managed realtime watch on a gadget: the overseer registers a fresh
   * AgentSelfLoopback with the gadget's subscribeAgent() and re-asserts it at every
   * subsequent turn end, so the chat is woken by the gadget's user-driven state changes
...
* To follow a Gadget's state changes in real time, use \`watchGadget\` (stop with \`unwatchGadget\`). This requires the Gadget's server.js to extend the platform kit: \`import { WatchableGadget } from "./gadget-kit.js";\` and \`export class Gadget extends WatchableGadget\`. The kit owns the subscription lifecycle — do NOT write subscribeAgent/unsubscribeAgent yourself, and never store an agent callback in the Gadget. The only thing you add is \`await this.notifyAgents(state)\` after state changes, optionally tagged: \`this.notifyAgents(state, {event: "move"})\` — chats can then subscribe selectively via watchGadget's events parameter.
* To push state to the Gadget's open UIs in real time, call \`await this.broadcast(state)\` (same kit) after state changes; client.js receives it via \`__platform.onUpdate(fn => ...)\`. Delivery survives server restarts and reconnects — prefer it over storing browser callback stubs in server.js (those die with the isolate). Plain \`subscribe(callback)\` remains for fine-grained event streams that are not state snapshots.
...
\`self\` underlies the platform's realtime collaboration, but you do not manage subscriptions with it by hand: call the \`watchGadget\` tool and the platform registers, maintains, and re-asserts the subscription for you (see the system prompt's design tips). Passing \`self\` to a Gadget's subscribe-style RPC yourself still works, but the platform's turn-end upkeep only covers watches established through the tool — so prefer the tool.
`.trim();
...
Optionally pass \`events\` to subscribe selectively: an array of event tags, and you are woken only by notifications the Gadget tagged with one of them (notifyAgents(state, {event: "win"})). Use this when you are an observer/coach rather than the opponent ("只在胜负时叫醒我" → events: ["win", "lose"]), or to ignore noise like resets. Notes: the filter binds only to TAGGED notifications — if the Gadget never tags its notifyAgents calls, you keep receiving everything (the filter is inert until the Gadget is updated to tag), so if the Gadget should tag events, add the tags to its notifyAgents calls in the same turn you watch with a filter. As the interactive opponent you should stay unfiltered, since every move must wake you. To stop following (the user says stop, or you hand control to the Gadget's built-in AI), call unwatchGadget — do NOT call the Gadget's unsubscribeAgent yourself; the platform would re-subscribe you at the end of the turn.
`.trim();

let UNWATCH_GADGET_TOOL_DESCRIPTION = `
Stop following a Gadget in real time (undoes watchGadget): the platform unsubscribes and you will no longer be woken by this Gadget's changes. Use it when the user ends the interactive session ("我自己来/你别跟了"), when you hand the Gadget's control to its own built-in automation, or when the interaction is clearly over. The user can re-establish realtime collaboration later by asking; you would then call watchGadget again.
`.trim();
...
export async function runAgent(
          ...
          for (let toolCall of msg.toolCalls) {
            ...
            try {
              if (toolCall.error) {
                ...
                case "watchGadget":
                case "unwatchGadget":
                  // The live call already registered/removed the platform-managed watch;
                  // the watch's own turn-end upkeep keeps it consistent from here.
                  toolOutput = {text: toolCall.output ?? ""};
                  break;
                default:
  ...
  let tools: Record<string, AgentTool> = {
    watchGadget: defineTool({
      name: "watchGadget",
      label: "Watch gadget in real time",
      description: WATCH_GADGET_TOOL_DESCRIPTION,
    ...
    unwatchGadget: defineTool({
      name: "unwatchGadget",
      label: "Stop watching gadget",
      description: UNWATCH_GADGET_TOOL_DESCRIPTION,
      parameters: Type.Object({
        gadget: Type.String({
          description: "Name of the Gadget binding to stop following.",
        }),
      }),
      execute: async (toolCallId, {gadget}) => {
        try {
          ...
          if (!entry) throw new Error(`There is no binding named "${gadget}" in your env.`);
          if (entry.type !== "workpiece") {
            throw new Error(`"${gadget}" is not a Gadget.`);
          }
          return toolResult(await hooks.unwatchGadget(chatId, entry.id, `${gadget}`));
        } catch (error) {
          return toolResult(`unwatchGadget failed: ${error instanceof Error ? error.message : String(error)}`, {isError: true});
        }
      }
    }),
